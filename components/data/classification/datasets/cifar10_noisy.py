...
class CIFAR10Dataset_Noisy(datasets.CIFAR10):
    def __init__(
        ...
        if not train:
            ...
            noise_ratio = 0
            Warning.warn("Noisy ratio is set to 0 for validation set")
    ...
    def __getitem__(self, index):
        image, true_target = super().__getitem__(index)
        if self.noise_type is not None and self.noise_ratio > 0:
            target = self.add_noise(true_target)
        else:
            target = true_target

        return image, target, {"index": index, "true_target": true_target}
    ...
    def _add_symmetric_noise(self):
        ...
        # 创建一个新的标签数组
        new_targets = np.array(self.targets)
